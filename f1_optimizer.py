...
def get_expected_race_pace(session_key):
    ...
    # Look for lap-duration column first
    lap_time_col = None
    if "lap_duration" in df.columns:
        ...
        # If sectors are present, sum them up
    ...
    df = df[df[lap_time_col] > 0]  # drop zero or negative
    df[lap_time_col] = pd.to_numeric(df[lap_time_col], errors="coerce")
    df = df[df[lap_time_col].notnull()]

    # Remove unusually slow laps (> 200 seconds)
    df = df[df[lap_time_col] < 200]
    if df.empty:
        raise Exception("No valid lap times found after filtering.")

    # Group by driver_number and compute mean and std
    driver_stats = (
        df.groupby("driver_number")[lap_time_col]
        .agg(["mean", "std", "count"])
        .reset_index()
    )

    # Keep only drivers with at least 3 laps
    driver_stats = driver_stats[driver_stats["count"] >= 3]
    if driver_stats.empty:
        raise Exception("No drivers with at least 3 laps found.")

    # Remove outliers per driver (laps > 2 std dev away)
    filtered_laps = []
    for _, driver_stat in driver_stats.iterrows():
        ...
        if not valid.empty:
            filtered_laps.append(
                {"driver_number": dn, "average_lap_time": valid.mean()}
            )

    if not filtered_laps:
        raise Exception("No valid lap data remaining after outlier filtering.")

    # Build final DataFrame and sort by fastest average lap time
...
def get_user_configuration():
    ...
    if config["use_fp2_pace"]:
        while True:
            try:
                sk = input("FP2 session key from OpenF1 API: ").strip()
                config["fp2_session_key"] = int(sk)
                break
            except ValueError:
        ...
        w_choice = input("Select pace weight (1-3) [2]: ").strip()
        config["pace_weight"] = weight_map.get(w_choice, 0.30)
        ...
        mod_choice = input("Select modifier type (1-2) [1]: ").strip()
        config["pace_modifier_type"] = modifier_map.get(mod_choice, "conservative")
...
class F1VFMCalculator:
    ...
    def _calculate_base_vfm(self, race_data_file, entity_type, weights):
        ...
        if self.scheme == "trend_based":
            race_df = self._calculate_trend_based_vfm(
                race_df, entity_col, race_columns, num_races
            )
        else:
        ...
        if entity_type.lower() == "driver":
            if self.scheme == "trend_based":
                result_df = race_df[
                    ["Driver", "Team", "Cost", "VFM", "Performance_Trend", "Weights_Used"]
                ]
            else:
            ...
            if self.scheme == "trend_based":
                result_df = race_df[
                    ["Constructor", "Cost", "VFM", "Performance_Trend", "Weights_Used"]
                ]
            else:
    ...
    def _load_driver_number_mapping(self):
        ...
        try:
            ...
            print(
                "No driver mapping file found. "
                "Create 'driver_mapping.csv' with columns: driver_number, driver_name, team_name"
            )
            return None
        except:
            return None
    ...
    def _remove_outliers(self, df, entity_col, race_columns):
        ...
        for ent in df_clean[entity_col].unique():
            ...
            for rc in race_columns:
                ...
                if not np.isnan(v) and (v < lb or v > ub):
        ...
        for ent in df[entity_col].unique():
            ...
            if len(pts) >= 3:
                ...
                if slope > threshold:
                    ...
                    base_w = [
                        0.8 + (0.2 * i / (len(idxs) - 1)) for i in range(len(idxs))
                    ]
                    trends[ent] = "Stable"
                ...
                if len(pts) == 1:
                    ...
                    min_w = 0.7
                    base_w = [
                        min_w + ((1.0 - min_w) / (len(pts) - 1)) * i for i in range(len(pts))
                    ]
    ...
    def run(self):
        ...
        constructor_vfm = self.calculate_vfm(
            constructor_race_file, constructor_vfm_file, "constructor"
        )
...
class F1TrackAffinityCalculator:
    ...
    def run(self):
        ...
        constructor_race_columns = [
            col for col in constructor_points.columns if col.startswith("Race")
        ]

        driver_points_clean = self._remove_outliers_advanced(
            driver_points, "Driver", race_columns
        )
        constructor_points_clean = self._remove_outliers_advanced(
            constructor_points, "Constructor", constructor_race_columns
        )

        driver_performance = self._prepare_performance_data(
            driver_points_clean, race_columns, "Driver"
        )
        constructor_performance = self._prepare_performance_data(
            constructor_points_clean, constructor_race_columns, "Constructor"
        )

        driver_circuit_performance = self._merge_track_data(
            driver_performance, race_calendar, track_characteristics
        )
        constructor_circuit_performance = self._merge_track_data(
            constructor_performance, race_calendar, track_characteristics
        )

        (
            driver_perf_encoded,
            constructor_perf_encoded,
            track_chars_encoded,
        ) = self._encode_categoricals(
            driver_circuit_performance,
            constructor_circuit_performance,
            track_characteristics,
        )

        char_importance = self._calculate_characteristic_importance(track_chars_encoded)

        driver_char_affinity = self._calculate_enhanced_affinity(
            driver_perf_encoded, "Driver", char_importance
        )
        constructor_char_affinity = self._calculate_enhanced_affinity(
            constructor_perf_encoded, "Constructor", char_importance
        )
        ...
        driver_track_affinity = self._calculate_track_affinity(
            driver_char_affinity_df, track_chars_encoded
        )
        constructor_track_affinity = self._calculate_track_affinity(
            constructor_char_affinity_df, track_chars_encoded
        )

        driver_final_output = self._create_final_output(
            track_characteristics, driver_track_affinity
        )
        constructor_final_output = self._create_final_output(
            track_characteristics, constructor_track_affinity
        )

        driver_char_affinity_df.round(3).to_csv(
            f"{self.base_path}driver_characteristic_affinities.csv"
        )
        driver_final_output.round(3).to_csv(
            f"{self.base_path}driver_affinity.csv", index=False
        )
        constructor_char_affinity_df.round(3).to_csv(
            f"{self.base_path}constructor_characteristic_affinities.csv"
        )
        constructor_final_output.round(3).to_csv(
            f"{self.base_path}constructor_affinity.csv", index=False
        )
    ...
    def _prepare_performance_data(self, df, race_columns, entity_type):
        ...
        melted = df.melt(
            id_vars=id_vars, var_name="Race", value_vars=race_columns, value_name="Points"
        )
    ...
    def _encode_categoricals(
        self, driver_perf, constructor_perf, track_chars
    ):
        ...
        for col in categorical_cols:
            if col in track_encoded.columns:
                le = LabelEncoder()
                track_encoded[col + "_encoded"] = le.fit_transform(track_encoded[col].fillna(""))
                encoders[col] = le
        ...
        for col in categorical_cols:
            if col in driver_enc.columns and col in encoders:
                driver_enc[col + "_encoded"] = encoders[col].transform(driver_enc[col].fillna(""))
            if col in constructor_enc.columns and col in encoders:
                constructor_enc[col + "_encoded"] = encoders[col].transform(constructor_enc[col].fillna(""))
    ...
    def _calculate_characteristic_importance(self, track_encoded):
        ...
        for char in char_cols:
            if char in track_encoded.columns:
                var = np.var(track_encoded[char].values)
    ...
    def _calculate_enhanced_affinity(self, df, entity_col, char_importance):
        ...
        for ent in df_valid[entity_col].unique():
            ...
            for c1, c2 in interaction_pairs:
                ...
                if (
                    c1 in ent_data.columns
                    and c2 in ent_data.columns
                    and ent_data[c1].notna().sum() >= 2
                    and ent_data[c2].notna().sum() >= 2
                ):
                    ...
                    scores[inter_name] = inter_corr * conf_w * 0.5
                else:
                    scores[inter_name] = 0.0
    ...
    def _calculate_robust_correlation(self, x, y):
        ...
        return max(corrs, key=abs) if corrs else 0.0
    ...
    def _calculate_track_affinity(self, char_affinity_df, track_chars_encoded):
        ...
        for ent in char_affinity_df.index:
            ...
            for _, track in track_chars_encoded.iterrows():
                ...
                # Base char contributions
                for char in base_cols:
                    if char in track_chars_encoded.columns and char in ent_aff:
                        val = track[char]
                        aff = ent_aff.get(char, 0.0)
                ...
                for inter in interaction_cols:
                    if inter in ent_aff:
                        ...
                        if c1 in track_chars_encoded.columns and c2 in track_chars_encoded.columns:
                            v1 = track[c1]
                            v2 = track[c2]
                            ...
                            if c2 == "Corners":
                                ...
                                n2 = v2 / m2 if m2 > 0 else 0.0

                            n1 = np.clip(n1, 0.0, 1.0)
                            n2 = np.clip(n2, 0.0, 1.0)
                            inter_val = n1 * n2
                            aff_i = ent_aff.get(inter, 0.0)
    ...
    def _create_final_output(self, track_characteristics, track_affinity):
        ...
        affinity_df = pd.DataFrame(track_affinity).T.fillna(0.0)
...
class F1TeamOptimizer:
    ...
    def load_data(self):
        ...
        try:
            ...
            s1 = self.race_calendar_df[
                self.race_calendar_df["Race"] == self.step1_race
            ]
            s2 = self.race_calendar_df[
                self.race_calendar_df["Race"] == self.step2_race
            ]
            if s1.empty or s2.empty:
                raise ValueError("Race information not found in calendar.csv")
            ...
            # Prepare track-specific affinity adjustments
            self._prepare_track_affinities()

            # Calculate current team cost and max budget
    ...
    def _prepare_track_affinities(self):
        ...
        # Keep original VFM for reference
        ...
        for step, circuit in [(1, self.step1_circuit), (2, self.step2_circuit)]:
            # Driver affinities
            row = self.track_affinity_df[
                self.track_affinity_df["Circuit"] == circuit
            ]
            if not row.empty:
            ...
            row2 = self.constructor_affinity_df[
                self.constructor_affinity_df["Circuit"] == circuit
            ]
            if not row2.empty:
                for col in constructor_affinity_cols:
                    ...
                    if name2 in self.constructors_df["Constructor"].values:
                        affinity2 = (
                            row2.iloc[0][col]
                            if pd.notna(row2.iloc[0][col])
                            else 0.0
                        )
                        idx2 = self.constructors_df[
                            self.constructors_df["Constructor"] == name2
                        ].index[0]
                        base_vfm2 = self.constructors_df.loc[idx2, "VFM_Original"]
                        adj_vfm2 = base_vfm2 * (
                            1 + affinity2 * self.affinity_weight
                        )
    ...
    def _get_team_data(self, drivers, constructors, step):
        ...
        d_df = self.drivers_df[
            self.drivers_df["Driver"].isin(drivers)
        ].copy()
        c_df = self.constructors_df[
            self.constructors_df["Constructor"].isin(constructors)
        ].copy()
    ...
    def evaluate_team(self, drivers, constructors, step):
        ...
        Returns:
          points (float), parts per million (float), cost (float), boost_driver (str)
        """
        ...
        if cost > self.max_budget:
            cache[key] = (-1.0, -1.0, cost, None)
            return -1.0, -1.0, cost, None
        ...
        for total in range(max_swaps + 1):
            for d_swaps in range(min(total + 1, len(current_drivers) + 1)):
                ...
                for out_d in itertools.combinations(current_drivers, d_swaps):
                    for out_c in itertools.combinations(
                        current_constructors, c_swaps
                    ):
                        patterns.append((out_d, out_c))

        return patterns

    def evaluate_swap_pattern(
        self,
        pattern,
        current_drivers,
        ...
        for new_ds in itertools.combinations(available_drivers, len(out_d)):
            ...
            for new_cs in itertools.combinations(
                available_constructors, len(out_c)
            ):
                ...
                if pts > best_result["points"]:
                    swaps = [
                        ("Driver", old, new) for old, new in zip(out_d, new_ds)
                    ]
                    swaps += [
                        ("Constructor", old, new) for old, new in zip(out_c, new_cs)
                    ]
                    best_result = {
                        "points": pts,
                        "ppm": ppm,
    ...
    def optimize_step(self, current_drivers, current_constructors, max_swaps, step):
        ...
        patterns = self.generate_swap_patterns(
            current_drivers, current_constructors, max_swaps
        )
        avail_d = [
        ...
        avail_c = [
            c
            for c in self.constructors_df["Constructor"]
            if c not in current_constructors
        ]
        ...
        base_pts, base_ppm, base_cost, base_boost = self.evaluate_team(
            current_drivers, current_constructors, step
        )
        if base_pts > 0:
            best_result = {
                "points": base_pts,
        ...
        if self.config["use_parallel"] and len(patterns) > 50:
            ...
            for p in patterns:
                r = self.evaluate_swap_pattern(
                    p, current_drivers, current_constructors, avail_d, avail_c, step
                )
                if r["points"] > best_result["points"]:
                    best_result = r
    ...
    def run_dual_step_optimization(self):
        ...
        print(
            f"\nOptimizing for {self.step1_race} ({self.step1_circuit}) "
            f"and {self.step2_race} ({self.step2_circuit})"
        )
        print(
            f"Current team - Step 1: {base_s1[0]:.1f} points, "
            f"Step 2: {base_s2[0]:.1f} points"
        )
        ...
        for i, pat in enumerate(patterns1):
            ...
            s2_res = self.optimize_step(
                s1_res["drivers"], s1_res["constructors"], self.config["step2_swaps"], 2
            )
            if (
                s2_res["points"] > best["final_points"]
                or (
                    s2_res["points"] == best["final_points"]
                    and s1_res["points"] > best["step1_points"]
                )
            ):
                best = {
                    "step1_result": s1_res,
    ...
    def print_results(self, optimization_result, base_s1, base_s2):
        ...
        if s1 and s1["swaps"]:
            ...
            for etype, old, new in s1["swaps"]:
                if etype == "Driver":
                    od = self.drivers_df[
                        self.drivers_df["Driver"] == old
                    ].iloc[0]
                    nd = self.drivers_df[
                        self.drivers_df["Driver"] == new
                    ].iloc[0]
                    print(f"  {old} → {new} (${od['Cost_Value']}M → ${nd['Cost_Value']}M)")
                else:
                    oc = self.constructors_df[
                        self.constructors_df["Constructor"] == old
                    ].iloc[0]
                    nc = self.constructors_df[
                        self.constructors_df["Constructor"] == new
                    ].iloc[0]
        ...
        if s2:
            ...
            print(
                f"Final budget used: ${s2['cost']:.1f}M / "
                f"${self.max_budget:.1f}M"
            )
        ...
        print("-" * 80)
        print(
            f"Total improvement: {optimization_result['final_points'] - base_s2[0]:.1f} points"
        )
        print(
            f"Patterns evaluated: {self.performance_stats['patterns_evaluated']:,}"
        )
        ...
        if self.config.get("use_fp2_pace", False):
            ...
            for d in self.config["current_drivers"]:
                row = self.drivers_df[self.drivers_df["Driver"] == d]
                if not row.empty and "Pace_Score" in row.columns:
                    ps = row.iloc[0].get("Pace_Score", 0)
                    pm = row.iloc[0].get("Pace_Modifier", 1.0)
                    if ps > 0:
                        print(f"  {d}: Pace score {ps:.1f}, VFM modifier {pm:.2f}x")
